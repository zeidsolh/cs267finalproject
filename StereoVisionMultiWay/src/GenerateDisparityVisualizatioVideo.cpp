...
int main(int argc, char **argv)
{
    ...
    if (parser.has("help") || (!parser.has("leftImage")) || (!parser.has("rightImage")) || (!parser.has("algorithmName")))
    {
        parser.printMessage();
        return 1;
    }
    ...
    parameters.leftImageFilePath = parser.get<cv::String>("leftImage");
    parameters.rightImageFilePath = parser.get<cv::String>("rightImage");
    parameters.outputPath = std::string(parser.get<cv::String>("outputPath"));
    parameters.algorithmName = parser.get<cv::String>("algorithmName");
    ...
    auto computeTime = std::chrono::high_resolution_clock::now();
    cv::Mat disparityImage(leftImage.rows, leftImage.cols, CV_32FC1);

    // int num_iterations = 10000; // Process the same image 1000 times
    ...
    auto doneComputeTime = std::chrono::high_resolution_clock::now();
    ...
    auto duration1 = std::chrono::duration_cast<std::chrono::milliseconds>(computeTime - startTime).count();
    auto duration2 = std::chrono::duration_cast<std::chrono::milliseconds>(doneComputeTime - computeTime).count();
    auto duration3 = std::chrono::duration_cast<std::chrono::milliseconds>(endTime - doneComputeTime).count();
    auto duration4 = std::chrono::duration_cast<std::chrono::milliseconds>(endTime - startTime).count();

    std::cout << "Time taken to create disparity generator: " << duration1 << " ms" << std::endl;
    std::cout << "Time taken to compute disparity: " << duration2 << " ms" << std::endl;
    std::cout << "Time taken to write output image: " << duration3 << " ms" << std::endl;
    std::cout << "Total time taken: " << duration4 << " ms" << std::endl;
